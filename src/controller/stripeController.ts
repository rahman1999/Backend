...
const stripe = Stripe('sk_test_51Kw1FzSHIOotpbGUfCQsP0ReNRtq1QFLdg1hZMYsiQwZijdzO7E0SMx5mqCNyrPmtbByPopn8Jhdg9cyDEkrfLO900bDW0puST');
let endpointSecret;
endpointSecret = 'whsec_004080a6d6e5ea88fcd7221a04a02e45bde273e31ed5c462d5abd3f02d0f71ce';
...
let cartdet;
   ...
   await AppDataSource.getRepository(User).findOne({ where: { id:order } ,relations:['userid']})
   .then(result=>{
    console.log(result);
...
console.log('cartdet:',cartdet)
   cartdet.map((item)=>{
    ...
    }catch(err){console.log(err)}    // orderDetail.push({order_name,order_image,order_total});
  })
  console.log('cartfinal:',orderDetail)
   console.log('finaladdress:',address)
   console.log('finalorder:',order_email,order_phone,order,orderDetail.order_name)
   return
   const ordermodel=new Orderdata();
...
console.log('order:',ordermodel)
  ...
  const secret = 'whsec_004080a6d6e5ea88fcd7221a04a02e45bde273e31ed5c462d5abd3f02d0f71ce';
